 ...
 *   5. POST /control/trigger/:name — triggerAgent returns { sessionId } → 200
 ...
 *  10. POST /control/kill/:sessionId — killSession throws → 500
 ...
 *   - control/routes/control.ts: POST /control/trigger/:name — sessionId result → 200
 ...
 *   - control/routes/control.ts: POST /control/kill/:sessionId — throws → 500
...
describe("integration: control/routes/control.ts direct tests (no Docker required)", { timeout: 20_000 }, () => {
  ...
  describe("POST /control/trigger/:name", () => {
    ...
    it("returns 200 with sessionId when triggerAgent succeeds", async () => {
      const fakeInstanceId = "test-instance-abc123";
      const app = makeApp({
        triggerAgent: vi.fn(async () => ({ sessionId: fakeInstanceId })),
      });
      const res = await app.request("/control/trigger/my-agent", { method: "POST" });
      expect(res.status).toBe(200);
      const body = await res.json() as { success: boolean; sessionId: string };
      expect(body.success).toBe(true);
      expect(body.sessionId).toBe(fakeInstanceId);
    });
