...
Agents reference a webhook source by name and add filters in their `SKILL.md` frontmatter:
  ...
  webhooks:
    - source: mintlify-docs
      events:
        - build
      actions:
        - failed    # Only trigger on build failures
      projects:
        - my-docs
```
...
3. **Check event filters** — verify the `events`, `actions`, and other filter fields in the agent's `SKILL.md` frontmatter match the incoming event.
...
To handle more concurrent events, increase `scale` in the agent's `SKILL.md` frontmatter:
