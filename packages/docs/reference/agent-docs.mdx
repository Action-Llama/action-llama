...
The scheduler reads `SKILL.md` and sends the markdown body as the system prompt. Runtime configuration is loaded from `config.toml`. A preamble is prepended that teaches the agent behavioral guidelines for its [available tools](/reference/agent-tools) (locks, calls, status). The LLM then receives a user-message prompt with:
...
- **Use the `set_status` tool** to report progress updates to the TUI.
...
2. **Lock the issue** — `acquire_lock` with resource `"github://owner/repo/issues/N"`. If locked, skip it.
3. **Implement** — Clone the repo, create a branch, make changes, run tests.
4. **Open a PR** — Push and open a PR linking to the issue.
5. **Unlock** — `release_lock` with the same resource.
...
- **Subagent call**: The `<skill-subagent>` block contains context from the calling agent. Do what was requested and use the `return_value` tool to send back results.
```
