...
Agents reference these by name in their `SKILL.md` frontmatter:
...
Override `scale` or `timeout` for a specific agent without editing its `SKILL.md`. These values take precedence over SKILL.md frontmatter.

| Field | Type | Description |
|-------|------|-------------|
| `scale` | number | Concurrent runner limit for this agent (overrides SKILL.md `scale`) |
| `timeout` | number | Max runtime in seconds for this agent (overrides SKILL.md `timeout`) |

```toml
[agents.dev]
scale = 3       # Allow 3 concurrent dev agent runs
timeout = 1800  # 30 minute timeout for dev agent
```
