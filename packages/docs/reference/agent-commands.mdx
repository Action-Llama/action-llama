...
description: "Shell commands available inside agent containers — environment, signals, calls, and locks"
---
...
Persist environment variables across bash commands. Each bash command the agent runs starts in a fresh shell, so variables set with plain `export` are lost between commands.

```bash
al-export <NAME> <value>
al-export -f
```

**First bash command — persist variables:**

```bash
al-export REPO "acme/app"
al-export ISSUE_NUMBER 42
```

**Later bash command — source the export file, then use the variables:**
...
`al-export` appends `export NAME='value'` lines to an export file. `al-export -f` prints that file path. To use the persisted variables in a fresh shell, source the file with:
...
The `wait_for_trigger` tool lets an agent suspend mid-conversation and wait for a specific trigger before resuming. While waiting, the container is paused and the runner slot is released.

### `wait_for_trigger`

Suspend and wait for a webhook event or agent trigger. This is a scheduler tool (not a shell command) — the agent calls it like any other tool.
...
The agent processes a `pull_request.opened` webhook, reviews the PR, then waits for it to merge:
...
```
Wait failed: Wait timeout expired
```
...
1. When a PR is opened, review the code and leave comments
2. Use `wait_for_trigger` to wait for the PR to be merged:
   - type: webhook, source: github, event: pull_request, match: action=closed
3. When the PR merges, run the deployment pipeline
4. Use `set_status` to update the dashboard with deployment progress
```

See [Agents — Wait & Resume](/concepts/agents#wait--resume) for architecture details.
