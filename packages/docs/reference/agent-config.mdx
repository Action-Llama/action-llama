...
See [Optimizing Agents](/guides/optimizing-agents) for a guide on using hooks effectively.

### How it works

1. Commands run **sequentially** in the order they appear in `SKILL.md` frontmatter
2. Commands run **inside the container** (or host process in `--no-docker` mode) after credential/env setup
