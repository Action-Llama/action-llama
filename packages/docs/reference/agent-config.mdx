...
# Optional: override the project default harness for this agent
[harness]
type = "claude"              # "pi" (default) or "claude"
...
# When scale > 1, use acquire_lock/release_lock in your actions to coordinate
scale = 2
...
# Optional: max wait timeout for wait_for_trigger in seconds (default: global defaultWaitTimeout or 1800)
waitTimeout = 3600
...
| `harness` | table | No | Harness override for this agent. Falls back to project `[harness]`, then defaults to `pi`. |
...
| `scale` | number | No | Number of concurrent runs allowed (default: 1). Set to `0` to disable the agent. Use `acquire_lock`/`release_lock` in your actions to coordinate instances. See [Resource Locks](/concepts/resource-locks). |
...
| `waitTimeout` | number | No | Maximum wait timeout for `wait_for_trigger` in seconds. Falls back to global `defaultWaitTimeout` (default: 1800 / 30 minutes). See [Wait & Resume](/concepts/agents#wait--resume). |
...
|-------|------|---------|-------------|
| `type` | string | Inherits project `[harness]` or `"pi"` | `"pi"` or `"claude"` |
