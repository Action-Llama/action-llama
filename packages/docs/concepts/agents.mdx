...
Agent authors reference the tools naturally in their `SKILL.md` (e.g. `acquire_lock` with a resource URI). The agent learns what the tools do from the preamble.

See [Agent Tools](/reference/agent-tools) for the complete tool reference.
...
1. **Trigger fires** — a cron tick, webhook event, manual `al run`, or `call_agent` from another agent.
...
10. **Signals are processed** — the agent uses the `set_status` and `return_value` tools to communicate with the scheduler during the session.
...
- [Agent Tools](/reference/agent-tools) — transport and scheduler tools
