...
5. **Hooks run** — if `hooks.pre` steps are defined, they execute sequentially (clone repos, fetch data, run shell commands) to stage context before the LLM starts. See [Dynamic Context](/guides/dynamic-context).
