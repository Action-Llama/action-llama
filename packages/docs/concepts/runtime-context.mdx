...
Environment variables: Plain `export` does not persist across bash commands. Use `al-export NAME value`, then start later commands with `. "$(al-export -f)"`.
...
Describes the filesystem constraints. In Docker mode, the agent learns that `/tmp` is writable and the root filesystem is read-only. In [host-user mode](/reference/agent-config#runtime), the agent's working directory is `/tmp/al-runs/<instance-id>/`. The `al-export` command persists environment variables in both modes.

See [Environment Commands](/reference/agent-commands#environment-commands) for `al-export` details, and [Container Filesystem](/concepts/agents#container-filesystem) for the full mount table.
