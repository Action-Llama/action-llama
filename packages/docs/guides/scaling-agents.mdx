...
2. For each issue:
   - LOCK("github://owner/repo/issues/123")
   ...
   - UNLOCK("github://owner/repo/issues/123")
```
...
When done: `release_lock` with the same resource releases the lock.
...
- [Agent Tools — Locking](/reference/agent-tools#locking) — lock tool reference and guidelines
