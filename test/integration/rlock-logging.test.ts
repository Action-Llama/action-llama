...
describe.skipIf(!DOCKER)("integration: rlock end-to-end", { timeout: 180_000 }, () => {
  ...
  it("rlock acquires a lock successfully", async () => {
      ...
      agents: [
        {
          ...
          testScript: [
            ...
            "",
            "# Acquire lock",
            'RESULT=$(rlock "test://integration-test/test-resource")',
            "",
            "# Verify JSON ok:true",
            ...
            "",
            "# Release",
            'runlock "test://integration-test/test-resource"',
            "exit 0",
          ].join("\n"),
        },
      ],
    ...
    const lockAcquire = harness.events.waitFor(
      "lock",
      (e) => e.action === "acquire" && e.ok && e.resourceKey === "test://integration-test/test-resource",
      60_000,
    );
      ...
      agents: [
        {
          ...
          testScript: [
            ...
            WAIT_FOR_GATEWAY,
            "",
            'rlock "test://integration-test/contested-lock"',
            "sleep 10",
            'runlock "test://integration-test/contested-lock"',
            "exit 0",
          ].join("\n"),
        },
        {
          ...
          testScript: [
            ...
            "set +e",
            'RESULT=$(rlock "test://integration-test/contested-lock")',
            "RC=$?",
            "set -e",
            "",
            ...
            '  runlock "test://integration-test/contested-lock" || true',
            "  exit 0",
            "fi",
            "",
      ...
      agents: [
        {
          ...
          testScript: [
            ...
            'rlock "test://integration-test/leaked-resource"',
            "exit 0",
          ].join("\n"),
        },
      ],
