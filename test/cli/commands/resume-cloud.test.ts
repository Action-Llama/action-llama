...
describe("resume --cloud", () => {
  ...
  it("resumes entire scheduler via cloud gateway", async () => {
    ...
    mockCloudGatewayFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ message: "Scheduler resumed." }),
    });
    ...
    expect(mockCloudGatewayFetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ path: "/control/resume", method: "POST" }),
    );
    expect(output).toContain("Scheduler resumed.");
  });

  it("resumes a specific agent via cloud gateway", async () => {
    tmpDir = makeCloudProject();
    ...
    mockCloudGatewayFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ message: "Agent dev resumed." }),
    });
