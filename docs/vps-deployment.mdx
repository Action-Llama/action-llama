...
To push only a single agent without restarting the scheduler, pass the agent name: `al push dev -E production`. The running scheduler detects the change and hot-reloads the agent automatically.
