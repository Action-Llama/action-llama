...
This webhook will trigger agents when documentation builds fail, allowing them to:
- Access build logs via the Mintlify API (using `mintlify_token` credential)
