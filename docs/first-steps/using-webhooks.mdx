...
[webhooks.my-github]
type = "github"
credential = "MyOrg"        # we'll create this credential next
```

## 2. Create the webhook secret credential

```bash
npx al creds add github_webhook_secret:MyOrg
```

Enter a random secret string (e.g. generate one with `openssl rand -hex 20`). Save this value — you'll need it when configuring GitHub.

## 3. Add a webhook trigger to your agent

In your agent's `agent-config.toml`, add a webhook trigger:
