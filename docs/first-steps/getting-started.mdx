...
```
agents/dev/
  SKILL.md             # YAML frontmatter (config) + markdown body (instructions)
```
...
| `agents/<name>/SKILL.md` | Agent config (YAML frontmatter) + instructions (markdown body) |
...
- [Optimizing Agents](/guides/optimizing-agents) — use hooks to stage context
