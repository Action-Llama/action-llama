...
Credentials are stored in `~/.action-llama/credentials/<type>/<instance>/<field>`. Each credential type is a directory containing one file per field. Reference them in your agent's `SKILL.md` frontmatter by type name (e.g. `"github_token"`) for the `default` instance, or use `"type:instance"` for a named instance (e.g. `"git_ssh:botty"`).
...
1. **Configuration**: List credential types in your agent's `SKILL.md` frontmatter:
   ```yaml
   credentials:
     - github_token
     - git_ssh
...
Agents only receive credentials listed in their `SKILL.md` frontmatter:
