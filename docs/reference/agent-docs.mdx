---
title: "Agent Docs"
description: "ACTIONS.md, AGENTS.md, and CLAUDE.md — what they do and how to write them"
---

Action Llama uses three markdown files with distinct purposes. Only `ACTIONS.md` is required.

## ACTIONS.md

The system prompt that defines an agent's behavior. This is the most important file in an agent directory.

### What it is

`ACTIONS.md` lives inside each agent's directory (e.g. `agents/dev/ACTIONS.md`). At runtime, its contents are injected as the LLM's system prompt. Everything the agent does is driven by these instructions.

### How it's used

The scheduler reads `ACTIONS.md` and sends it as the system prompt. A preamble is prepended that teaches the agent its [language skills](/reference/agent-commands) (signals, calls, locks, credentials). The LLM then receives a user-message prompt with:
...
4. **Trigger context** — schedule, webhook payload, manual, or agent call context
...
- **Reference `<agent-config>` for parameter values.** Don't hardcode repo names or labels — put them in `[params]` and reference them.
...
1. **Find work** — If triggered by webhook, work on the issue from the trigger. If scheduled, search for open issues labeled with the label from `<agent-config>`.triggerLabel.
...
Your `ACTIONS.md` should handle different trigger types if the agent uses both:
...
- **Agent call**: The `<agent-call>` block contains context from the calling agent. Do what was requested and use `al-return` to send back results.
```
...
`CLAUDE.md` lives at the project root and provides instructions to AI coding assistants that may be editing the project's source code.
...
If you use Claude Code or similar AI development tools to work on your Action Llama project, put project conventions, build commands, and coding guidelines in `CLAUDE.md`.
