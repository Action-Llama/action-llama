...
The single file that defines an agent. YAML frontmatter at the top contains the agent's configuration (credentials, schedule, models, webhooks, hooks, params), and the markdown body is the system prompt that drives the agent's behavior. This is the most important file in an agent directory.

### What it is

`SKILL.md` lives inside each agent's directory (e.g. `agents/dev/SKILL.md`). The frontmatter is parsed for configuration, and the markdown body is injected as the LLM's system prompt at runtime. Everything the agent does is driven by these instructions.

### How it's used

The scheduler reads `SKILL.md`, extracts the frontmatter for configuration, and sends the markdown body as the system prompt. A preamble is prepended that teaches the agent its [language skills](/reference/agent-commands) (signals, calls, locks, credentials). The LLM then receives a user-message prompt with:

1. **`<skill-config>`** — JSON of the `params` table from the `SKILL.md` frontmatter
