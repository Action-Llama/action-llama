...
The single file that defines an agent. YAML frontmatter at the top contains the agent's configuration (credentials, schedule, model, webhooks, hooks, params), and the markdown body is the system prompt that drives the agent's behavior. This is the most important file in an agent directory.
