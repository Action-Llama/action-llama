...
# Optional: hooks — run before or after the LLM session
# Each hook runs a built-in provider to stage data or perform cleanup
hooks:
  pre:
...
| `model.thinkingLevel` | string | No | Thinking budget level: off, minimal, low, medium, high, xhigh. Only relevant for Claude Sonnet/Opus. |
...
Hooks run mechanical data-staging or cleanup tasks around the LLM session. Pre-hooks (`hooks.pre`) run after credentials are loaded but before the LLM session starts — they fetch data, clone repos, or run commands to prepare the workspace so the agent starts with everything it needs. Post-hooks (`hooks.post`) run after the session completes.

See [Optimizing Agents](/guides/optimizing-agents) for a guide on using hooks effectively.

### How it works

1. Steps run **sequentially** in the order they appear in `SKILL.md` frontmatter
2. Steps run **inside the container** (or host process in `--no-docker` mode) after credential/env setup
3. Providers write files to disk — your SKILL.md instructions reference the staged files
...
Each entry in `hooks.pre` or `hooks.post` has these fields:
...
- There is no per-hook timeout — hooks are bounded by the container-level timeout
...
The `model` section is optional — agents inherit the default model from the project's `config.toml`. Only add `model` to the `SKILL.md` frontmatter if you want to override the default for that specific agent.
