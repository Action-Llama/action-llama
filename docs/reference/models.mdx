...
Action Llama supports 8 LLM providers. Define named models in `config.toml` under `[models.<name>]`, then reference them by name in each agent's `SKILL.md` frontmatter. Agents list models in priority order — the first is the primary, the rest are fallbacks tried automatically on rate limits.
...
Each agent can use a different model. Define all models in `config.toml`, then reference them by name in each agent's `SKILL.md`:

```
config.toml        → [models.sonnet], [models.gpt4o], [models.groq-llama]
dev/SKILL.md       → models: [sonnet]
reviewer/SKILL.md  → models: [gpt4o]
devops/SKILL.md    → models: [groq-llama]
```
...
LLM credentials are loaded automatically based on the models referenced in the agent's `models` list — they do not need to be listed in the agent's `credentials` array. The `credentials` array is for runtime credentials the agent uses during execution (GitHub tokens, SSH keys, etc.).
