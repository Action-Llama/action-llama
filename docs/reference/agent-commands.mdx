...
Agents running in Docker containers have access to shell commands for signaling the scheduler, calling other agents, and coordinating with resource locks. These commands are installed at `/tmp/bin/` and taught to agents via a preamble injected before `ACTIONS.md`.

Agent authors write shorthand in `ACTIONS.md` (e.g. `LOCK("github issue acme/app#42")`). The preamble teaches the agent the underlying shell commands.
...
Resource locks prevent multiple agent instances from working on the same resource. The agent preamble teaches these as `LOCK(...)`, `UNLOCK(...)`, and `HEARTBEAT(...)` shorthand. The underlying shell commands are `rlock`, `runlock`, and `rlock-heartbeat`.
...
### Lock shorthand in ACTIONS.md

In your `ACTIONS.md`, write the lock operations naturally:
...
The preamble teaches the agent that `LOCK(...)` maps to `rlock`, `UNLOCK(...)` to `runlock`, and `HEARTBEAT(...)` to `rlock-heartbeat`.
