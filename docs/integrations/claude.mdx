...
- How `SKILL.md` files work (YAML frontmatter for config, markdown body for instructions)
...
- **"Create a new agent that monitors Sentry for new errors and opens GitHub issues"** — Claude writes a `SKILL.md` with the right frontmatter (credentials, schedule, webhooks) and instructions.
- **"Add webhook support to the dev agent for issue comments"** — Claude edits the frontmatter to add a webhook trigger and updates the instructions to handle both scheduled and webhook triggers.
- **"Change the reviewer agent to use GPT-4o"** — Claude updates the `model` field in the frontmatter.
