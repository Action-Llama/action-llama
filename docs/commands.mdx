...
Deploy your project to a server over SSH. Requires a `[server]` section in your environment file. See [VPS Deployment](/vps-deployment) for server configuration.
...
```bash
al env set production -p .     # Bind project to "production"
al env set -p .                # Unbind project from any environment
```
...
Tear down a provisioned environment. Stops containers, cleans up remote credentials, and optionally deletes the VPS instance if it was provisioned via `al env prov`.

```bash
al env deprov staging -p .
```
