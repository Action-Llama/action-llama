...
Requires `gcloud` CLI with project admin permissions. See [Cloud Run docs](/cloud-run) for full setup.
...
Requires AWS CLI with IAM admin permissions. See [ECS docs](/ecs) for full setup.

**VPS** (`cloud.provider = "vps"`):

Pushes all local credentials to the VPS filesystem over SSH. No IAM roles — SSH access implies full access. See [VPS docs](/vps-deployment) for full setup.
