...
description: "Deploy agents to AWS ECS, Google Cloud Run, or a VPS"
---
...
See [Cloud Run docs](/cloud-run) for prerequisites, full setup walkthrough, and troubleshooting.
...
See [ECS docs](/ecs) for prerequisites, full setup walkthrough, and troubleshooting.
...
On AWS, agents with `timeout <= 900` automatically route to Lambda for faster cold starts. Agents with longer timeouts use ECS Fargate. See [ECS docs](/ecs#per-agent-timeout-and-lambda-routing) for details.
