...
| `mintlify_webhook_secret` | `secret` | Shared secret for Mintlify webhook verification | _(used by gateway)_ |
...
When using cloud runtimes, credentials are automatically pushed to the cloud secret manager by `al doctor -c` or `al setup cloud`. See [Cloud Run docs](/cloud-run) and [ECS docs](/ecs) for details.
