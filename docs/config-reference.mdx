...
| `timeout` | number | `900` | Default max container runtime in seconds. Individual agents can override this with `timeout` in their `agent-config.toml`. On AWS ECS, agents with effective timeout <= 900s automatically route to Lambda for faster cold starts. See [agent timeout docs](/agent-config-reference#timeout). |
...
See [Cloud Run docs](/cloud-run) for full setup.
...
See [ECS docs](/ecs) for full setup.
