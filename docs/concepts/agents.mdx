...
The contents of `ACTIONS.md`, prepended with a **preamble** that teaches the agent its [language skills](#language-skills).
...
| **Calls** | `al-call`, `al-check`, `al-wait` | Agent-to-agent calls with return values |
...
5. **Preflight runs** — if `[[preflight]]` steps are defined, they execute sequentially (clone repos, fetch data, run shell commands) to stage context before the LLM starts. See [Optimizing Agents](/guides/optimizing-agents).
6. **LLM session starts** — the model receives `ACTIONS.md` as system prompt and the assembled user prompt.
...
Each container has a self-termination timer controlled by `timeout` in `agent-config.toml` (falls back to `local.timeout` in `config.toml`, then 900 seconds). If the timer fires, the process exits with code 124. This is a hard kill — there is no graceful shutdown.

See [agent-config.toml timeout](/reference/agent-config#timeout) for configuration.
...
- [agent-config.toml Reference](/reference/agent-config) — all config fields
- [Agent Docs](/reference/agent-docs) — ACTIONS.md, AGENTS.md, CLAUDE.md
