...
export function buildLockSkill(): string {
  const lines = [
    ...
    `  -d '{"secret":"'$SHUTDOWN_SECRET'","resource":"<resource>","key":"<key>"}'`,
    "```",
    "",
    "### UNLOCK(resource, key)",
    ...
    "- Use descriptive resource names: `githubIssue`, `githubPR`, `deployment`",
    '- Use unique keys: `owner/repo#42`, `service-name`',
    "</skill-lock>",
  ];
  return lines.join("\n");
}
